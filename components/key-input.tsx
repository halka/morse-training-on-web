...
export default function KeyInput() {
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>電鍵入力</CardTitle>
          <CardDescription>
            オーディオケーブルで接続された電鍵からの入力を検出します。
        ...
        <CardContent>
          <div className="items-center">
            <Button onClick={isListening ? stopAudio : initAudio} variant={isListening ? "destructive" : "default"}>
              {isListening ? (
                <>
                  <MicOff className="mr-2 h-4 w-4" /> 停止
                </>
              ) : (
                <>
          ...
          <div className="space-y-2">
            ...
            <div className="p-4 min-h-16 bg-muted rounded-md font-mono break-all">
              {currentMorse || "電鍵入力を待機中..."}
            </div>
          </div>

          <div className="space-y-2">
            <div className="font-medium">デコードされたテキスト:</div>
            <div className="p-4 min-h-16 bg-muted rounded-md break-all">
              {decodedText || "デコードされたテキストがここに表示されます"}
            </div>
          </div>

          <div className="flex gap-2">
            <Button onClick={decodeCurrentMorse} disabled={!currentMorse}>
              デコード
            </Button>
